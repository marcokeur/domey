...
void mqtt_sn_publish(uint16_t topic_id, const char * data, uint8_t retain)
{
    ...
    if (data_len <= sizeof(packet.data)) {   
      ...
      packet.flags += MQTT_SN_SHORT_TOPIC_TYPE;                                
...
static void doReport() { 
  ...
  topic_id = (LIGHT_SENSOR_ID << 8) + NODE_ID;
  //sprintf(buff, "%d", payload.light);
  buff[0] = 0x31; //(char)payload.light;
  mqtt_sn_publish(topic_id, buff, false);
  
  delay(1000);
  
  topic_id = (HUMIDITY_SENSOR_ID << 8) + NODE_ID;
  buff[0] = 0x32; //(char)payload.humi;
  mqtt_sn_publish(topic_id, buff, false);

  delay(1000);

  topic_id = (TEMPERATURE_SENSOR_ID << 8) + NODE_ID;
